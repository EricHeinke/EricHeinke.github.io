...
public class MyWorld extends World
{
    ...
    char myColor = 'b';
    String loadFile = "brain-v0.txt";
    String saveFile = "brain-v0.txt";
    int count = 0;
    ...
    public int ai()
    {
        ...
        for(int i=0; i<7; i++)
        {
            if(ran <= tmp2[i])
            {
                ...
                if(checkWinner() == 'b')
                {
                    for(int a=0; a<memoryHash.size(); a++)
                    {
                        brain.get(memoryHash.get(a))[memoryPos.get(a)] += 1;
                    }
                }
                else if(checkWinner() == 'r')
                {
                    for(int a=0; a<memoryHash.size(); a++)
                    {
                        brain.get(memoryHash.get(a))[memoryPos.get(a)] -= 1;
    ...
    public void playerRedStrategy(){
        ...
        playerRed = "Random";  //put your name here
        ...
        if(false)
        {
            try (Scanner input = new Scanner(System.in)){
                colToPlace = Integer.parseInt(input.nextLine());
            }
            catch(Exception e){}
        }
